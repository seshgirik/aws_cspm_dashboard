...
Answer: This scenario assesses understanding of baseline security and change management:
	•	Baseline Configuration: Define secure baselines (e.g. CIS Benchmarks) for all OS (Windows, Linux) and network devices (Cisco, Juniper). Use automated tools (Ansible, Chef, SSM, or SCCM) to enforce these baselines (disable unnecessary services, enforce password policies, enable firewalls). Document baselines for audit (ISO 27001 requires documented operating procedures).
	•	Patch Management Process: Establish a formal process: subscribe to vendor security bulletins (Microsoft, RedHat, Cisco). Use a patch management system (e.g. WSUS/Intune for Windows, yum/apt with automated repos for Linux, or cloud-native patching tools like AWS SSM Patch Manager).
